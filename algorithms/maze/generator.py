...
class Maze:
    ...
    def makePP(self,mode = "RGB", colorWall = 0, colorFloor = 1, pixelSizeOfTile = 10, ):
        """
        ...
        test = []
        for a in range(len(self.mazeList) * 2 + 1):
            test.append([])
            for b in range(len(self.mazeList) * 2 + 1):
                test[a].append(WALL)
        ...
        for row in self.mazeList: #Iterates over all tiles
                for tile in row:
                    ...
                    drawImage.rectangle([x, y, x +  pixelSizeOfTile  -1, y +  pixelSizeOfTile  -1], fill = colorX)
                    testX = int(x / pixelSizeOfTile)
                    testY = int(y / pixelSizeOfTile)
                    test[testX][testY] = PATH

                    if "N" in tile.connectTo:
                        drawImage.rectangle([x, y -  pixelSizeOfTile , x +  pixelSizeOfTile  - 1, y - 1], fill = colorX)
                        testX2 = int(x / pixelSizeOfTile)
                        testY2 = int(y / pixelSizeOfTile - 1)
                        test[testY2][testX2] = PATH

                    if "S" in tile.connectTo:
                        drawImage.rectangle([x, y +  pixelSizeOfTile , x +  pixelSizeOfTile  - 1, y +  pixelSizeOfTile  +  pixelSizeOfTile  - 1], fill = colorX)
                        testY3 = int(y / pixelSizeOfTile + 1)
                        testX3 = int(x / pixelSizeOfTile)
                        test[testY3][testX3] = PATH


                    if "W" in tile.connectTo:
                        drawImage.rectangle([x -  pixelSizeOfTile , y, x - 1, y +  pixelSizeOfTile  - 1], fill = colorX)
                        testX4 = int(x / pixelSizeOfTile - 1)
                        testY4 = int(y / pixelSizeOfTile)
                        test[testY4][testX4] = PATH

                    if "E" in tile.connectTo:
                        drawImage.rectangle([x +  pixelSizeOfTile , y, x +  pixelSizeOfTile  +  pixelSizeOfTile  - 1, y +  pixelSizeOfTile  - 1], fill = colorX)
                        testX5 = int(x / pixelSizeOfTile + 1)
                        testY5 = int(y / pixelSizeOfTile)
                        test[testY5][testX5] = PATH

        # pp.pprint(test)
        return image, test #returns an image object
    ...
    def saveImage(self,image,name = None,format = None):
        ...
        #     size = ( 'abc'  * (self.sizeX * 2 + 1),  10  * (self.sizeY * 2 + 1))
...
def get_maze(show = False, size = [5, 5]):
    newMaze = Maze(size[0], size[1])
